...
DWORD WINAPI PrivacyModeThreadProc(LPVOID lpParam) {
    while (privacyModeEnable) {
        for (size_t i = 0; i < privacyModeMonitors.size(); ++i) {
            HANDLE monitorHandle = privacyModeMonitors[i].hdl;  // Assuming 'hdl' is the member representing the handle
            DWORD pdwCurrentValue;
            DWORD pdwMaximumValue;
            if (privacyModeLoadLib.GetVCPFeatureAndVCPFeatureReplyFunc()(monitorHandle, PowerMode, NULL, &pdwCurrentValue, &pdwMaximumValue)) {
                if (pdwCurrentValue == PowerOn) {
                    privacyModeLoadLib.SetVCPFeatureFunc()(monitorHandle, PowerMode, PowerOff);
                }
            }
        }
        Sleep(500);
    ...
    for (size_t i = 0; i < privacyModeMonitors.size(); ++i) {
        HANDLE monitorHandle = privacyModeMonitors[i].hdl;  // Assuming 'hdl' is the member representing the handle
        privacyModeLoadLib.SetVCPFeatureFunc()(monitorHandle, PowerMode, PowerOn);
    }
    return 1;
}
