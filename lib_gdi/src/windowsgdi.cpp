...
void DWAGDIDestroyWindow(int id){
	...
	if (dwawin!=NULL){
		...
		PostMessageW(dwawin->hwnd, WM_CLOSE, (WPARAM)0, (LPARAM)0);
	}
}
...
wchar_t* DWAGDIGetClipboardText(){
    ...
    wchar_t* emptyString = (wchar_t*)malloc(sizeof(wchar_t));
    *emptyString = L'\0'; // Set the empty string to a null terminator
    return emptyString;
}
